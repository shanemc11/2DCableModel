...
#==============================================================================
# #Plot the lambda and error from the fits

# #initalize a dataframe for the data   
# param_data = pd.DataFrame()
# #import the data to the above data frame
# param_data = pd.read_excel(datadir + '221128_exp_fit_parameters.xlsx')

# #setup aspects of the data to plot as a bar chart
# x_pos = np.arange(len(param_data['region']))
# diff_app = param_data['lambda_mean']
# diff_app_error = param_data['lambda_CI']
...
#     ax.set_ylabel(u'Decay length, $\lambda$ (${\mu}m$)', fontsize=24)
#     ax.set_xticks(x_pos[:])
#     # ax.set_xticklabels(param_data['region'][:], fontsize=20)
#     plt.ylim([0,3.0])
#     plt.tight_layout()
#     # plt.savefig('221128_lambda_param_barplot.svg') 
    
#=============================================================================
# #set color palette to use for plots
# cmap = ["#7fb800", "#ffb400", "#7fb800", "#ffb400"]

# #Fit each N and make categorical plot of fitted parameters
# bin1_N = bin1.groupby(['n'])
# bin1_pars_df = pd.DataFrame()

# for name, group in bin1_N:
#     df = pd.DataFrame()
#     pars, covar = curve_fit(exp_fit, group['cell_len_norm'], \
#                                          group['int_norm_rescale'], \
#                                   absolute_sigma = False, p0=(1, 0.3, 1))
#     df['lambda'] = pars
#     df['n'] = name
#     df['bin'] = 'bin1'
...
#     df2 = pd.DataFrame()
#     pars2, covar2 = curve_fit(exp_fit, group['Distance_(microns)'],\
#                                   group['int_norm_rescale'], \
#                                   absolute_sigma = False, p0=(1, 1, 1))
#     df2['lambda'] = pars2
#     df2['n'] = name
...
#     bin1_pars_df = bin1_pars_df.append(df.iloc[1], ignore_index=True)
#     bin1_pars_df = bin1_pars_df.append(df2.iloc[1], ignore_index=True)
    
# #Fit each N and make categorical plot of fitted parameters
# bin3_N = bin3.groupby(['n'])
# bin3_pars_df = pd.DataFrame()

# for name, group in bin3_N:
#     df = pd.DataFrame()
#     pars, covar = curve_fit(exp_fit, group['cell_len_norm'], \
#                                          group['int_norm_rescale'], \
#                                   absolute_sigma = False, p0=(1, 0.3, 1))
...
#     bin3_pars_df = bin3_pars_df.append(df.iloc[1], ignore_index=True)
#     bin3_pars_df = bin3_pars_df.append(df2.iloc[1], ignore_index=True)

# frames = [bin1_pars_df, bin3_pars_df]   

# all_par_df = pd.concat(frames) 
...
#     sns.swarmplot(x='scale', y='lambda', hue='bin', data = all_par_df,
#                   linewidth=0.5,edgecolor='k', zorder=0, size=10, dodge=True)   
        
#     # ax = sns.stripplot(x='region', y='lambda', data = exptN_parm, size=15,\
#     #                    order = o, color='grey', edgecolor='k', marker="s",\
#     #                     linewidth=1)
...
#     ax = sns.pointplot(x='scale', y='lambda', hue='bin', data = all_par_df,
#                        capsize = 0.3, join=False, color='k', dodge=0.4)
        
#     plt.ylabel(u'Decay length, $\lambda$ (${\mu}m$)', fontsize=ft)
#     plt.xlabel(None)
#     ax.set(xticks=[])
#     ax.tick_params('both', length=5, which='both')
...
#     # plt.ylim([0, 22])
#     plt.legend([],[], frameon=False)
#     # plt.savefig('210927_cable_number_categorical.svg') 
